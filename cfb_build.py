...
ESPN_PATH = "/apis/site/v2/sports/football/college-football/scoreboard?groups=80&limit=400&seasontype=2&week={week}&dates={season}"
...
def get_events(week):
    """Try each ESPN host (twice); keep only events that really belong to this week."""
    if os.environ.get("CFB_ESPN_FILE"):
        srcs = [("file", os.environ["CFB_ESPN_FILE"].format(week=week))]
    else:
        srcs = [(h, h + ESPN_PATH.format(week=week, season=S)) for h in ESPN_HOSTS] * 2
    for host, u in srcs:
        try:
            if host == "file":
                js = json.load(open(u))
            ...
            evs = [e for e in evs if (e.get("week") or {}).get("number") in (None, week)]
            msg = f"week {week}: {len(evs)} games from {host.replace('https://', '')}"
            print("  ESPN", msg); FEED_LOG.append(msg)
            if evs: return evs
        except Exception as e:
            msg = f"week {week}: {type(e).__name__} {getattr(e, 'code', '')} from {host.replace('https://', '')}".replace("  ", " ")
            print("  ESPN", msg); FEED_LOG.append(msg)
    return []
...
data = dict(season=S, week=W, demo=demo, feed_log=FEED_LOG[-6:], built=dt.datetime.now(dt.timezone.utc).strftime("%Y-%m-%d %H:%M UTC"),
